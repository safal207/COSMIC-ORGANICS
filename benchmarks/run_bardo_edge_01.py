...
def build_report(speed_rounds: int = 3) -> dict:
        ...
        for target_index, target in enumerate(targets):
            ...
            for source in sources:
                ...
                for round_index in range(speed_rounds):
                    ...
                    if not (
                        conventional["transition_records"]
                        == bardo["transition_records"]
                    ):
    ...
    for name, bucket in per_system.items():
        summary[name] = {
            ...
            "median_edge_reconstruction_work": (
                median(bucket["edge_work"]) if bucket["edge_work"] else None
            ),
            "replay_correctness": (
                bucket["replay_pass"] / trials if bucket["edge_work"] else None
            ),
            "minimum_phase_change_coverage": (
                min(bucket["coverage"]) if bucket["coverage"] else None
            ),
            "source_destination_relation_coverage": (
                sum(bucket["relation_coverage"]) / len(bucket["relation_coverage"])
                if bucket["relation_coverage"]
                else None
            ),
            "phantom_transitions": bucket["phantom"] if bucket["edge_work"] else None,
            "missing_transitions": bucket["missing"] if bucket["edge_work"] else None,
    ...
    explicit_edge_supported = controls_ok and (
        ...
        >= protocol["acceptance"][
            "explicit_edge_reconstruction_work_reduction_min_fraction"
        ]
    )
    ...
    candidate_metrics = {
        "metadata_fraction_vs_conventional": (
            bardo["transition_metadata_bytes"]
            / max(1, conventional["transition_metadata_bytes"])
        ),
        "reconstruction_work_fraction_vs_conventional": (
            bardo["median_edge_reconstruction_work"]
            / max(1, conventional["median_edge_reconstruction_work"])
        ),
        "runtime_fraction_vs_conventional": (
            bardo["median_runtime_round_ns"]
            / max(1, conventional["median_runtime_round_ns"])
        ),
    }
    threshold = 1.0 - protocol["acceptance"]["bardo_specific_improvement_min_fraction"]
    bardo_specific_supported = bardo_semantics_ok and any(
        fraction <= threshold for fraction in candidate_metrics.values()
    )
    ...
    return {
        ...
        "decision": decision,
        "claim_boundary": (
            "Classical deterministic software/lattice evidence only. Runtime is "
            "a Python-runner measurement, not hardware or physical-energy evidence."
        ),
    }
...
def main() -> None:
    ...
    if report["decision"] == "REJECTED_CONTROL_FAILURE":
        raise SystemExit("BARDO-EDGE-01 control failure")
