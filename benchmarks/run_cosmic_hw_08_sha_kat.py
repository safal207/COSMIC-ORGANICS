...
def tb_text() -> str:
    lines = [
        ...
        "  block_data=blk; block_valid=1; @(posedge clk); block_valid=0;",
        "  while(!digest_valid) @(posedge clk);",
        "  if(digest_data!==exp) begin $display(\"FAIL idx=%0d got=%064h exp=%064h\",idx,digest_data,exp); $fatal(1); end",
        "  @(posedge clk);",
        "end endtask",
        "integer i; initial begin repeat(3) @(posedge clk); reset=0;",
    ]
...
def main() -> None:
    ...
    proc = subprocess.run(["vvp", str(out)], check=True, text=True, capture_output=True)
