...
def incident_schedule() -> tuple[list[float], ...]:
    ...
    for tick in range(24):
        ...
        # The primary localized event moves nominal cells A -> M -> C.
        if 1 <= tick <= 3:
            assignments.append((PRIMARY_INCIDENT, 0.90))

        # A second independent event demonstrates disjoint sparse frontiers.
        if 8 <= tick <= 10:
            assignments.append((SECONDARY_INCIDENT, 0.90))

        # Bounded negative drive models an explicit remediation action.
        if 12 <= tick <= 14:
            assignments.append((PRIMARY_INCIDENT, -0.90))
        if 17 <= tick <= 19:
            assignments.append((SECONDARY_INCIDENT, -0.90))
...
def _mutated_proof_is_rejected(kernel: CommittedProofKernel) -> bool:
    ...
    mutated_node = replace(
        first,
        claim=replace(first.claim, phase_after=wrong_phase),
    )
