"""Experimental temporal-memory extension for MORPHOS.

The retained activation variable is an algorithmic state used to test temporal
integration. It is not calibrated physical energy and must not be interpreted
as one without a named material/device mapping.
"""
...
class TemporalLattice:
    """Synchronous lattice with a decaying signed activation accumulator."""
    ...
    def __init__(
        ...
        *,
        ...
        self.cells = [TemporalCell(phase=phase) for phase in phases]
        self.tick = 0
    ...
    def step(self, stimuli: float | Sequence[float]) -> None:
        ...
        for index, (cell, stimulus) in enumerate(zip(self.cells, stimulus_values)):
            ...
            self.max_abs_activation_seen = max(
                self.max_abs_activation_seen, abs(activation)
            )

            direction = 0
            if activation >= self.config.crystallize_threshold:
                direction = 1
            elif activation <= -self.config.amorphize_threshold:
                direction = -1
    ...
    def snapshot(self) -> dict:
        return {
            "protocol_version": "MORPHOS-T1/0.1",
            "tick": self.tick,
