...
Run the robustness and external-baseline suites:
...
The current toy tasks are reproducible but do not establish algorithmic uniqueness: simple external baselines tie the three original positive tasks.

Run state capacity and T2 repair:
...
State-capacity testing exposed a mixed-state dead zone. T2 introduces a separate `M` relaxation threshold and maps 243 repair configurations.

Current T2 result:

- `85 / 243` configurations preserve all five locked gates;
- `8` eliminate the declared mixed-state dead zone;
- a capacity-matched candidate ties majority CA at `4.0` bits and `37.5%` recovery, but costs more transitions;
- a recovery-biased candidate reaches `45.24%` recovery at `3.585` bits, again at higher transition cost;
- `0` configurations dominate majority CA on capacity + recovery + transition cost simultaneously.

So the dead zone is partially repairable, but **a distinct computational advantage is still unproven**.

All six result artifacts are committed under `results/` and regenerated byte-for-byte in CI.
